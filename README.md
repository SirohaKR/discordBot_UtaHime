...
- `/기본스타일설정` — 이 서버의 **기본 스타일 참조 이미지**를 등록(강도/정보추출량 지정 가능). 등록해두면 이후 `/그림생성`에서 스타일참조를 따로 안 붙여도 자동으로 이 이미지가 Vibe Transfer로 적용됨. 채팅 채널의 🎨 버튼으로 만드는 그림에도 동일하게 적용됨(채팅 중 첨부한 이미지가 없을 때). 그 요청에서만 다른 스타일을 쓰고 싶으면 스타일참조를 직접 첨부하면 그게 우선됨.
- `/기본스타일해제` — 등록된 서버 기본 스타일 참조를 해제.
- **캐릭터 프리셋** (유저별): NovelAI는 이미지로 "이 캐릭터 얼굴 그대로 유지해줘" 같은 정체성 참조를 지원하지 않아서(Vibe Transfer는 화풍/색감 전이지 얼굴 고정이 아님), 대신 캐릭터 고정 특징을 텍스트로 저장해뒀다가 매번 재사용하는 방식으로 일관성을 잡는다.
  - `/캐릭터저장 이름:<이름> 특징:<머리색/눈색/헤어스타일 등 고정 묘사>` — 캐릭터별 고정 특징을 저장(같은 이름으로 다시 저장하면 덮어씀).
  - `/그림생성`에 `캐릭터:<이름>`을 지정하면 그 특징은 유지한 채 이번 프롬프트는 장면/포즈/의상 묘사로만 반영해서 합성함 (자동완성 지원).
  - `/캐릭터목록`, `/캐릭터삭제`로 관리.
...
- 그림 아이디어를 대화로 다듬다가 Claude가 "지금 만들어도 되겠다" 판단한 순간에만(내부 마커 신호) 답장에 **🎨 이 대화로 그림 만들기** 버튼이 붙음 — 매번 뜨지 않고 필요할 때만 나타나서 일상 대화를 방해하지 않음. 누르면 지금까지 나눈 대화 내용을 태그로 변환해서 바로 이미지를 생성함 (`/그림생성`과 동일한 결과 버튼들이 붙음). 기본 비율/모델/등급으로 생성되며, 이후 "다시 생성" 버튼으로 이어서 조정 가능.
...
| `!동기화` | `!sync` | 이 서버에 슬래시 명령어를 즉시 동기화. 전역 동기화는 디스코드 쪽 캐시 때문에 서버마다 반영까지 최대 1시간 걸릴 수 있는데(봇 재시작 시 "홈 서버"만 예외로 즉시 반영됨), 새 서버에 봇을 초대했거나 명령어를 방금 바꿨는데 안 보일 때 그 서버에서 실행하면 바로 반영됨 |
