# -*- coding: utf-8 -*-
"""이미지 생성(/그림생성) 전용 채널 설정 저장소 (SQLite, playlists.db 공유)."""
...
def get_channel(guild_id: int):
    with _get_conn() as conn:
        row = conn.execute(
            "SELECT channel_id FROM image_settings WHERE guild_id = ?", (guild_id,)
        ).fetchone()
...
def set_channel(guild_id: int, channel_id: int) -> None:
    with _get_conn() as conn:
        conn.execute(
            """
            INSERT INTO image_settings (guild_id, channel_id) VALUES (?, ?)
...
def clear_channel(guild_id: int) -> None:
    with _get_conn() as conn:
        conn.execute("DELETE FROM image_settings WHERE guild_id = ?", (guild_id,))
