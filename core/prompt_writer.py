...
- 완전히 새로운 요소(포즈, 소품, 배경 등)를 추가해달라는 요청이면 적절한 위치(캐릭터 외형 뒤, 배경 태그 앞 등)에
  자연스럽게 끼워 넣으세요.
- 등장인물이 정확히 한 명인데(1girl/1boy 등) 현재 태그 목록에 "solo"가 빠져있으면 추가해주세요 (인원수
  태그 바로 뒤 정도가 적당). 이게 없으면 정체불명의 다른 사람 손/팔이 화면에 환각으로 나오는 경우가 흔합니다.
...
- 없애달라는 대상이 [현재 태그 목록]에 애초에 없는 경우도 흔합니다 — 그건 사용자가 태그로 넣은 적 없는데
  결과 이미지에 원치 않게 나온 요소(예: 손 이상, 정체불명의 남의 손/팔 같은 생성 아티팩트)를 말하는
  겁니다. 이 경우 태그 목록에서 지울 건 없지만, 네거티브 후보에는 반드시 추가하세요. 예: "타인의 손
  제거"/"손 이상하게 나온 거 없애줘" -> 태그 목록은 그대로 두고 네거티브 후보에
  "another person's hand, extra hand, disembodied hand, wrist grab" 같은 관련 태그들을 추가.
