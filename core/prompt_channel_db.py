# -*- coding: utf-8 -*-
"""프롬프트 추천 전용 채널 설정 저장소 (SQLite, playlists.db 공유)."""
...
def _get_conn():
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS prompt_channel_settings (
...
def get_channel(guild_id: int):
    with _get_conn() as conn:
        row = conn.execute(
            "SELECT channel_id FROM prompt_channel_settings WHERE guild_id = ?", (guild_id,)
        ).fetchone()
...
def set_channel(guild_id: int, channel_id: int) -> None:
    with _get_conn() as conn:
        conn.execute(
            """
            INSERT INTO prompt_channel_settings (guild_id, channel_id) VALUES (?, ?)
...
def clear_channel(guild_id: int) -> None:
    with _get_conn() as conn:
        conn.execute("DELETE FROM prompt_channel_settings WHERE guild_id = ?", (guild_id,))
